...
public class GetProposalsBean {
    ...
    public void getProposals(Exchange exchange) throws Exception {
        ...
        List<String> serviceExternalId = getServiceExternalId(exchange, getExtProposalsRequest);
//        String posId = getPosId(exchange);
        String posId = "100000034";
        ...
        for (GetProposalsResponse getProposalsResponse : getProposalsResponses) {
            ...
            if ("0".equals(getProposalsResponse.getOffersCount())) {
                ...
                if ("AccountSummary".equals(presentationContext) || "ViewAllPromotions".equals(presentationContext)) {
                    ...
                    getProposalItemList.addAll(mapCMPResponseNBA(getProposalsResponse, serviceExternalId.get(counter), services.get(counter)));
                }
            }
            counter ++;
        }
    ...
    private List<String> getServiceExternalId(Exchange exchange, GetExtProposalsRequest getExtProposalsRequest) throws Exception {
        isMock = exchange.getProperty("MockComponentEndpoint", false, Boolean.class);
        ...
        List<String> serviceExternalIds = IntStream.range(0, numOfServices)
                .parallel()
                .mapToObj(counter -> {
                    try {
                        int serviceInternalId = services.get(counter).getServiceInternalId();
                        int serviceInternalIdReset = services.get(counter).getServiceInternalIdResets();
                        ...
                        sb.append(uri)
                                ...
                                .append("%2C")
                                .append(serviceInternalIdReset);
                        Exchange newExchange = new DefaultExchange(exchange.getContext());
                        newExchange.getIn().setHeaders(exchange.getIn().getHeaders());

                        log.info("Thread: {}, Calling microservice for serviceInternalId: {}, serviceInternalIdResets: {}", Thread.currentThread().getName(), serviceInternalId, serviceInternalIdReset);

                        productCatalogCommonBean.callMicroService(exchange, newExchange, sb.toString(), null, CommonConstants.HTTPMethod.GET, null, isMock);

                        if (Objects.nonNull(newExchange.getException())) {
                            log.error("Error retrieving serviceExternalId for serviceInternalId: {}, serviceInternalIdResets: {}", serviceInternalId, serviceInternalIdReset);
                            throw new ValidationException("Error Retrieving serviceExternalId");
                        } else {
                            HashMap response = newExchange.getIn().getBody(HashMap.class);
                            log.debug("Thread: {}, customerfacingservices response for serviceInternalId: {}, serviceInternalIdResets: {}: {}", Thread.currentThread().getName(), serviceInternalId, serviceInternalIdReset, response);
                            String externalId = (String) response.get("serviceExternalId");

                            if (Objects.nonNull(externalId)) {
                                log.info("Thread: {}, Retrieved serviceExternalId: {} for serviceInternalId: {}, serviceInternalIdResets: {}", Thread.currentThread().getName(), externalId, serviceInternalId, serviceInternalIdReset);
                                return externalId;
                            } else {
                                log.error("Failed to retrieve serviceExternalId for serviceInternalId: {}, serviceInternalIdResets: {}", serviceInternalId, serviceInternalIdReset);
                                throw new ValidationException(SERVICE_EXTERNAL_ID_FAILURE);
                            }
                        }
                    } catch (Exception e) {
    ...
    private ArrayList<GetProposalItem> mapCMPResponseNBA(GetProposalsResponse getProposalsResponse, String serviceExternalId, SubscriberIdentifier service) {
        ...
        for (Offer offerItem : offers) {
            ...
            proposalItem.setTargetedObjectInternalId(service.getServiceInternalId());
            proposalItem.setTargetedObjectInternalIdResets(service.getServiceInternalIdResets());
            proposalItem.setProposalId(optInKeywordValue);
