package com.amdocs.bl.productcatalog.custom.bean;
...
public class GetProposalsBeanFinal {
    ...
    public void getProposals(Exchange exchange) throws Exception {
        ...
        Triple<List<String>, List<Integer>, List<Integer>> serviceIds = getServiceExternalAndInternalIds(exchange, getExtProposalsRequest);

        List<String> serviceExternalIds = serviceIds.getLeft();
        List<Integer> serviceInternalIds = serviceIds.getMiddle();
        List<Integer> serviceInternalIdResets = serviceIds.getRight();

        String posId = getPosId(exchange);

        List<GetProposalsRequest> getProposalsRequests = IntStream.range(0, numOfServices)
                .mapToObj(counter -> setRequestBodyToCMP(serviceExternalIds.get(counter), posId))
                .collect(Collectors.toList());

        List<GetProposalsResponse> getProposalsResponses = getProposalsRequests.parallelStream()
                .map(request -> callGetAvailableOffersAndGetResponse(exchange, request))
                .collect(Collectors.toList());

        processProposalsResponses(exchange, getExtProposalsRequest, presentationContext, getProposalsResponses, serviceExternalIds, serviceInternalIds, serviceInternalIdResets);

        log.info("finish: getProposals");
    }

    private void processProposalsResponses(Exchange exchange, GetExtProposalsRequest getExtProposalsRequest, String presentationContext, List<GetProposalsResponse> getProposalsResponses, List<String> serviceExternalId, List<Integer> serviceInternalIds, List<Integer> serviceInternalIdResets) throws ValidationException {
        ...
        for (GetProposalsResponse getProposalsResponse : getProposalsResponses) {
            ...
            if ("0".equals(getProposalsResponse.getOffersCount())) {
                ...
                if ("AccountSummary".equals(presentationContext) || "ViewAllPromotions".equals(presentationContext)) {
                    ...
                    getProposalItemList.addAll(mapCMPResponseNBA(getProposalsResponse, serviceExternalId.get(counter), serviceInternalIds.get(counter), serviceInternalIdResets.get(counter)));
                }
            }
            counter ++;
        }
    ...
    private GetProposalsResponse callGetAvailableOffersAndGetResponse(Exchange exchange, GetProposalsRequest getProposalsRequest) {
        try {
    ...
    public String getPosId(Exchange exchange) throws Exception {
    ...
    private GetProposalsRequest setRequestBodyToCMP(String serviceExternalId, String posId) {
        ...
        String[] category = {NBO_TEEO, NBA_TEEO};
    ...
    private Triple<List<String>, List<Integer>, List<Integer>> getServiceExternalAndInternalIds(Exchange exchange, GetExtProposalsRequest getExtProposalsRequest) throws Exception {
        isMock = exchange.getProperty("MockComponentEndpoint", false, Boolean.class);
        ...
        log.info("start: getServiceExternalAndInternalIds");
        ...
        List<Integer> serviceInternalIds = new ArrayList<>();
        List<Integer> serviceInternalIdResets = new ArrayList<>();
        ...
        List<HashMap> accountDetails = newExchange.getIn().getBody(List.class);

        log.info("Microservice is called for accountid: {}", accountInternalId);

        IntStream.range(0, numOfServices).forEach(counter -> {
            try {
                if (Objects.nonNull(newExchange.getException())) {
                    log.error("Error retrieving serviceExternalId for accountId: {}", accountInternalId);
                    throw new ValidationException("Error Retrieving serviceExternalId");
                }
                else {
                    log.info("accountDetails: {}", accountDetails);

                    accountDetails.forEach(serviceObject -> {
                        String serviceExternalId = (String) serviceObject.get("serviceExternalId");
                        Integer serviceInternalId = (Integer) serviceObject.get("serviceInternalId");
                        Integer serviceInternalIdReset = (Integer) serviceObject.get("serviceInternalIdResets");

                        log.debug("Thread: {}, customerfacingservices serviceObject for serviceInternalId: {}, serviceInternalIdResets: {}: {}", Thread.currentThread().getName(), serviceInternalId, serviceInternalIdReset, accountDetails);

                        if (Objects.nonNull(serviceExternalId)) {
                            log.info("Thread: {}, Retrieved serviceExternalId: {} for serviceInternalId: {}, serviceInternalIdResets: {}", Thread.currentThread().getName(), serviceExternalId, serviceInternalId, serviceInternalIdReset);
                            log.info("Adding to the lists: {}", serviceObject);
                            serviceExternalIds.add(serviceExternalId);
                            serviceInternalIds.add(serviceInternalId);
                            serviceInternalIdResets.add(serviceInternalIdReset);
                        }
                    });
                }
            } catch (Exception e) {
        ...
        log.info("finish: getServiceExternalAndInternalIds");
        return Triple.of(serviceExternalIds, serviceInternalIds, serviceInternalIdResets);
    }
    ...
    private ArrayList<GetProposalItem> mapCMPResponseNBO(GetProposalsResponse getProposalsResponse, GetExtProposalsRequest getExtProposalsRequest) {
        ...
        for (Offer offerItem : offers) {
            if (NBA_TEEO.equals(offerItem.getProductOffering().getRecoOffersStrategy().getCategory())) {
                continue;
            }
            ...
            for (POAttribute poAttribute : poAttributes) {
                if (OPT_IN_KEYWORD.equals(poAttribute.getName())) {
                    optInKeywordValue = poAttribute.getValue();
                } else if (MARKETING_NAME_EN.equals(poAttribute.getName())) {
    ...
    private ArrayList<GetProposalItem> mapCMPResponseNBA(GetProposalsResponse getProposalsResponse, String serviceExternalId, Integer serviceInternalId, Integer serviceInternalIdReset) {
        ...
        for (Offer offerItem : offers) {
            ...
            List<POAttribute> poAttributes = offerItem.getProductOffering().getPoAttribute();
            for (POAttribute poAttribute : poAttributes) {
                if (MARKETING_NAME_EN.equals(poAttribute.getName())) {
                    marketingNameENValue = poAttribute.getValue();
                } else if (OPT_IN_KEYWORD.equals(poAttribute.getName())) {
                    optInKeywordValue = poAttribute.getValue();
            ...
            proposalItem.setTargetedObjectId(serviceExternalId);
            proposalItem.setTargetedObjectInternalId(serviceInternalId);
            proposalItem.setTargetedObjectInternalIdResets(serviceInternalIdReset.shortValue());
            proposalItem.setProposalId(optInKeywordValue);
